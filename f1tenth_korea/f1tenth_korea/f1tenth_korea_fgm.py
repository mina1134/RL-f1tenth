...
class Planner(Node):
    ...
    def preprocess_lidar(self, ranges, scan_range):
        ...
        return proc_ranges
    """
    def preprocess_lidar(self, scan, scan_range): 
        self.front_idx = (int(scan_range / 2))

        scan_origin = [0] * scan_range
        scan_filtered = [0] * scan_range

        for i in range(scan_range):
            scan_origin[i] = scan[i]
            scan_filtered[i] = scan[i]

        for i in range(scan_range - 1):
            if scan_origin[i] * self.FILTER_SCALE < scan_filtered[i + 1]:
            ...
            elif scan_filtered[i] > scan_origin[i + 1] * self.FILTER_SCALE:
    ...
    def find_best_point(self, scan):
        ...
        averaged_max_gap = np.convolve(scan[self.gap[0]:self.gap[1]], np.ones((self.gap[1]-self.gap[0])//2),
                                       'same')
    ...
    def scan_callback(self, scan_msg):
        ...
        while i < len(self.points):
            if dist2pt[i] > scan[point_indices[i]] - 0.2:
                start = i
                while dist2pt[i] > scan[point_indices[i]] - 0.2:
