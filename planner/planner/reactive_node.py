...
class ReactiveFollowGap(Node):
    ...
    def find_max_gap(self, free_space_ranges):
        ...
        for i in range(len(free_space_ranges)):
            if free_space_ranges[i] > self.max_gap_safe_dist: # != 0.0 leads to no obs
                ...
                streak = 0
        start_index = np.clip(start_index-bubble_index, 0, len(free_space_ranges)-1)
        end_index = np.clip(end_index+bubble_index, 0, len(free_space_ranges)-1)
    ...
    def lidar_callback(self, data):
        ...
        ranges = np.array(data.ranges[360:])
        ...
        # map steering_angle
        steering_angle = np.deg2rad(best_i * self.downsample_gap / 4.0 -45.0)
